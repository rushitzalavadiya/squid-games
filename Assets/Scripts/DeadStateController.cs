...
public class DeadStateController : MonoBehaviour, IInitializable
{
	...
	[SerializeField]
	private RelativeAimController m_relativeAimController;

	[SerializeField]
	private DirectAimController m_directAimController;

	[SerializeField]
	private CharacterMovement m_characterMovement;

	[SerializeField]
	private Strafer m_strafer;
	...
	public bool IsDead
	{
		...
		set
		{
			...
			if (m_isDead)
			{
				m_animator.SetTrigger("Death" + UnityEngine.Random.Range(1, 4));
			}
			if (m_relativeAimController != null)
			{
				m_relativeAimController.IsDead = m_isDead;
			}
			if (m_directAimController != null)
			{
				m_directAimController.IsDead = m_isDead;
			}
			if (m_characterMovement != null)
			{
				m_characterMovement.IsDead = m_isDead;
			}
			if (m_strafer != null)
			{
				m_strafer.IsDead = m_isDead;
			}
