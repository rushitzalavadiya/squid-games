...
namespace TinyJson
{
	public static class JSONParser
	{
		...
		internal static object ParseValue(Type type, string json)
		{
			...
			{
				decimal.TryParse(json, NumberStyles.Float, CultureInfo.InvariantCulture, out decimal result2);
				return result2;
			}
			if (json == "null")
			{
			...
			if (type.IsGenericType && type.GetGenericTypeDefinition() == typeof(List<>))
			{
				...
				IList list3 = (IList)type.GetConstructor(new Type[1]
				{
					typeof(int)
				}).Invoke(new object[1]
				{
			...
			if (type.IsGenericType && type.GetGenericTypeDefinition() == typeof(Dictionary<, >))
			{
				...
				for (int l = 0; l < list4.Count; l += 2)
				{
					if (list4[l].Length > 2)
					{
						...
						object value = ParseValue(type3, list4[l + 1]);
		...
		private static object ParseAnonymousValue(string json)
		{
			...
			if (char.IsDigit(json[0]) || json[0] == '-')
			{
				...
				int.TryParse(json, out int result2);
				return result2;
			}
			if (json == "true")
			{
		...
		private static object ParseObject(Type type, string json)
		{
			...
			if (!fieldInfoCache.TryGetValue(type, out Dictionary<string, FieldInfo> value))
			{
			...
			if (!propertyInfoCache.TryGetValue(type, out Dictionary<string, PropertyInfo> value2))
			{
			...
			for (int i = 0; i < list.Count; i += 2)
			{
				if (list[i].Length > 2)
				{
					...
					PropertyInfo value4;
					if (value.TryGetValue(key, out FieldInfo value3))
					{
